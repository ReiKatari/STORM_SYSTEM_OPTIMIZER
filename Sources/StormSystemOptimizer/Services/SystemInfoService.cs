    ...
    public class SystemInfoService
    {
        ...
        private HardwareDetailCategory GetCpuSpecs()
        {
            ...
            try
            {
                ...
                cat.Add("Топология CPU Sets", topo.ClassificationName);
                cat.Add("Распределение ядер", $"{topo.TotalPhysicalCores} ядер / {topo.TotalLogicalProcessors} потоков (SMT: {(topo.HasSmt ? "Активен" : "Отключен")})");
                if (topo.Domains.Count > 0)
                {
                    cat.Add("Домены L3 кэша", $"{topo.Domains.Count} домен(а)");
                }
            }
            catch { }
        ...
        public string ExportSpecsToPlainText(List<HardwareDetailCategory> specs)
        {
            ...
            sb.AppendLine("================================================================================");
            sb.AppendLine("         Сформировано через STORM Engine 2.1.1 • 100% Safe Optimization         ");
            sb.AppendLine("================================================================================");
            sb.AppendLine("             Конец отчета • STORM SYSTEM OPTIMIZER 2.1.1                        ");
            sb.AppendLine("================================================================================");
            return sb.ToString();
        }
        ...
        public async Task<string> ExportHtmlReportAsync()
        {
            ...
            sb.AppendLine("<h1>⚡ STORM SYSTEM OPTIMIZER 2.1.1 — Диагностический отчет</h1>");
            sb.AppendLine($"<p style='color:#64748B;'>Сформировано: {DateTime.Now:dd.MM.yyyy HH:mm:ss} • STORM Engine 2.1.1</p>");
            return sb.ToString();
        }
